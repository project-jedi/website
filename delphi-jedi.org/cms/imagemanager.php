<?php
// $Id: imagemanager.php,v 1.10 2003/07/08 12:38:08 okazu Exp $
//  ------------------------------------------------------------------------ //
...
if ($op == 'list') {
	...
	if ($catcount > 0) {
		...
		$catshow = !isset($HTTP_GET_VARS['cat_id']) ? 0 : intval($HTTP_GET_VARS['cat_id']);
		...
		ksort($catlist);
		$cat_options = '';
		foreach ($catlist as $value => $name) {
			$sel = '';
			if ($value == $catshow) {
				$sel = ' selected="selected"';
			}
			$cat_options .= '<option value="'.$value.'"'.$sel.'>'.$name.'</option>';
		}
