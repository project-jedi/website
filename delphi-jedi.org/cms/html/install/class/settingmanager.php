...
class setting_manager {
    ...
    function setting_manager($post=false){
        ...
        if($post){
            ...
            $this->xoops_url = (!empty($filepath)) ? $protocol.$HTTP_SERVER_VARS['HTTP_HOST']."/".$filepath : $protocol.$HTTP_SERVER_VARS['HTTP_HOST'];
        }
    }

    function readPost(){
        global $HTTP_POST_VARS;
        if(isset($HTTP_POST_VARS['database']))
            $this->database = $this->sanitizer->stripSlashesGPC($HTTP_POST_VARS['database']);
        if(isset($HTTP_POST_VARS['dbhost']))
            $this->dbhost = $this->sanitizer->stripSlashesGPC($HTTP_POST_VARS['dbhost']);
        if(isset($HTTP_POST_VARS['dbuname']))
            $this->dbuname = $this->sanitizer->stripSlashesGPC($HTTP_POST_VARS['dbuname']);
        if(isset($HTTP_POST_VARS['dbpass']))
            $this->dbpass = $this->sanitizer->stripSlashesGPC($HTTP_POST_VARS['dbpass']);
        if(isset($HTTP_POST_VARS['dbname']))
            $this->dbname = $this->sanitizer->stripSlashesGPC($HTTP_POST_VARS['dbname']);
        if(isset($HTTP_POST_VARS['prefix']))
            $this->prefix = $this->sanitizer->stripSlashesGPC($HTTP_POST_VARS['prefix']);
        if(isset($HTTP_POST_VARS['db_pconnect']))
            $this->db_pconnect = intval($HTTP_POST_VARS['db_pconnect']) > 0 ? 1 : 0;
        if(isset($HTTP_POST_VARS['root_path']))
            $this->root_path = $this->sanitizer->stripSlashesGPC($HTTP_POST_VARS['root_path']);
        if(isset($HTTP_POST_VARS['xoops_url']))
            $this->xoops_url = $this->sanitizer->stripSlashesGPC($HTTP_POST_VARS['xoops_url']);
    }
