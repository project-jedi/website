<?php
// $Id: readpmsg.php,v 1.9 2003/02/16 15:23:06 okazu Exp $
//  ------------------------------------------------------------------------ //
...
if ( !$xoopsUser ) {
	...
	if ( !empty($HTTP_POST_VARS['delete']) ) {
		$pm =& $pm_handler->get(intval($HTTP_POST_VARS['msg_id']));
	...
	$total_messages = !empty($HTTP_GET_VARS['total_messages']) ? intval($HTTP_GET_VARS['total_messages']) : 0;
	...
	if (empty($pm_arr)) {
		...
		echo $pm_arr[0]->getVar("subject")."<br /><form action='readpmsg.php' method='post' name='delete".$pm_arr[0]->getVar("msg_id")."'><table border='0' cellpadding='4' cellpadding='1' class='outer' width='100%'><tr><th colspan='2'>". _PM_FROM ."</td></tr><tr class='even'>\n";
