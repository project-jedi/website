<?php
// $Id: modulesadmin.php,v 1.27 2003/09/19 05:46:40 okazu Exp $
//  ------------------------------------------------------------------------ //
...
function xoops_module_uninstall($dirname)
{
	...
	if ($module->getVar('dirname') == 'system') {
		...
		if (!$module_handler->delete($module)) {
			...
			$tplfile_handler = xoops_gethandler('tplfile');
			$criteria = new CriteriaCompo(new Criteria('tpl_refid', $module->getVar('mid')));
			$criteria->add(new Criteria('tpl_type', 'module'));
			$templates =& $tplfile_handler->getObjects($criteria);
			...
			if (is_array($block_arr)) {
				...
				for ($i = 0; $i < $bcount; $i++) {
					...
					if ($block_arr[$i]->getVar('template') != ''){
						$criteria = new CriteriaCompo('tpl_type', 'block');
						$criteria->add(new Criteria('tpl_refid', $block_arr[$i]->getVar('bid')));
						$templates =& $tplfile_handler->getObjects($criteria);
...
function xoops_module_change($mid, $weight, $name)
{
	...
	if (!$module_handler->insert($module)) {
		$ret = "<p>".sprintf(_MD_AM_FAILORDER, "<b>".$module->getVar('name')."</b>")."&nbsp;"._MD_AM_ERRORSC."<br />";
		$ret .= $module->getHtmlErrors()."</p>";
		return $ret;
	}
	return "<p>".sprintf(_MD_AM_OKORDER, "<b>".$module->getVar('name')."</b>")."</p>";
}
