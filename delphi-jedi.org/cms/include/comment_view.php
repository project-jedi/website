<?php
// $Id: comment_view.php,v 1.15 2003/09/04 12:21:12 okazu Exp $
//  ------------------------------------------------------------------------ //
...
if (XOOPS_COMMENT_APPROVENONE != $xoopsModuleConfig['com_rule']) {
	...
	if ($com_itemid > 0) {
		$com_mode = isset($HTTP_GET_VARS['com_mode']) ? htmlspecialchars(trim($HTTP_GET_VARS['com_mode']), ENT_QUOTES) : '';
		if ($com_mode == '') {
		...
		if (!isset($HTTP_GET_VARS['com_order'])) {
			if (is_object($xoopsUser)) {
			...
			$com_order = intval($HTTP_GET_VARS['com_order']);
		}
		...
		$com_rootid = isset($HTTP_GET_VARS['com_rootid']) ? intval($HTTP_GET_VARS['com_rootid']) : 0;
