<?php
// $Id: image.php,v 1.4 2003/04/14 11:13:39 okazu Exp $
//  ------------------------------------------------------------------------ //
...
class XoopsImageHandler extends XoopsObjectHandler
{
    ...
    function insert(&$image)
    {
        ...
        if ($image->isNew()) {
            ...
            if (isset($image_body) && $image_body != '') {
                $sql = sprintf("UPDATE %s SET image_body = %s WHERE image_id = %u", $this->db->prefix('image_body'), $this->db->quoteString($image_body), $image_id);
                if (!$result = $this->db->query($sql)) {
