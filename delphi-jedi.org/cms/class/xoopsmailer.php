<?php
// $Id: xoopsmailer.php,v 1.14 2003/04/25 08:53:55 okazu Exp $
//  ------------------------------------------------------------------------ //
 ...
 * @version		$Revision: 1.14 $ - changed by $Author: okazu $ on $Date: 2003/04/25 08:53:55 $
 */
class XoopsMailer
{
	/**
...
//				if ($debug) {
//					$this->errors[] = _MAIL_FNAMENG;
//				}
//				return false;
//			}
			$from = $this->encodeFromName($this->fromName);
			//if ( $this->fromEmail == "" ) {
			//	$this->errors[] = _MAIL_FEMAILNG;
			//	return false;
			//}
			$this->headers[] = "From: ".$from." <".$this->fromEmail.">".$this->LE;
			if (!empty($this->priority)) {
				$this->headers[] = "X-Priority: " . $this->priority . $this->LE;
			}
			$this->headers[] = "X-Mailer: PHP/".phpversion().$this->LE;
			$this->headers[] = "Return-Path: ".$this->fromEmail;
			//foreach ($this->addHeaders as $ah) {
			//	$headers .= $ah.$this->LE;
			//}
