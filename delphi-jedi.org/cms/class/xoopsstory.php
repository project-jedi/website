<?php
// $Id: xoopsstory.php,v 1.10 2003/04/01 22:51:21 mvandam Exp $
//  ------------------------------------------------------------------------ //
...
class XoopsStory
{
	...
	function hometext($format="Show")
	{
		...
		case "Show":
			...
			if ($xoopsModuleConfig['news_lbrconversion'] == '1') {
				$breaks = 0;
			} else {
				$breaks = 1;
			}
			$hometext = $myts->displayTarea($this->hometext,$html,$smiley,$xcodes,1,$breaks);
			break;
		case "Edit":
	...
	function bodytext($format="Show")
	{
		...
		case "Show":
			...
			$bodytext = $myts->displayTarea($this->bodytext,$html,$smiley,$xcodes,1,$breaks);
			break;
		case "Edit":
