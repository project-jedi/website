<?php
// $Id: bloggerapi.php,v 1.6 2003/02/22 19:38:26 okazu Exp $
//  ------------------------------------------------------------------------ //
...
class BloggerApi extends XoopsXmlRpcApi
{
    ...
    function getUserInfo()
    {
        if (!$this->_checkUser($this->params[1], $this->params[2])) {
            ...
            $struct->add('url', new XoopsXmlRpcStruct($this->user->getVar('url')));
            $struct->add('email', $this->user->getVar('email'));
            $struct->add('lastname', '');
            $struct->add('firstname', $this->user->getVar('name'));
            $this->response->add($struct);
        }
    }
