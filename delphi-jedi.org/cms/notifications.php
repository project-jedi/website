<?php
// $Id: notifications.php,v 1.4 2003/04/01 04:31:17 okazu Exp $
//  ------------------------------------------------------------------------ //
...
if (isset($HTTP_POST_VARS['delete_ok'])) {
	$op = 'delete_ok';
}
if (isset($HTTP_POST_VARS['delete_cancel'])) {
	$op = 'cancel';
}
...
case 'delete_ok':
	...
	if (empty($HTTP_POST_VARS['del_not'])) {
		redirect_header('notifications.php', 2, _NOT_NOTHINGTODELETE);
	}
	include XOOPS_ROOT_PATH.'/header.php';
	$hidden_vars = array('uid'=>$uid, 'delete_ok'=>1, 'del_not'=>$HTTP_POST_VARS['del_not']);
...
case 'delete':
	...
	$notification_handler =& xoops_gethandler('notification');
	foreach ($HTTP_POST_VARS['del_not'] as $n_array) {
