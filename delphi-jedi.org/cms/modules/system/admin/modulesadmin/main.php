<?php
// $Id: main.php,v 1.23 2003/07/08 12:38:10 okazu Exp $
//  ------------------------------------------------------------------------ //
...
if ( $op == "confirm" ) {
	...
	foreach ($module as $mid) {
		...
		echo '<tr class="'.$class.'"><td align="center">'.stripslashes($oldname[$mid]);
		$newname[$mid] = trim($newname[$mid]);
		if ($newname[$mid] != $oldname[$mid]) {
			echo '&nbsp;&raquo;&raquo;&nbsp;<span style="color:#ff0000;font-weight:bold;">'.stripslashes($newname[$mid]).'</span>';
		}
		...
		<input type='hidden' name='oldname[".$mid."]' value='".$oldname[$mid]."' />
		<input type='hidden' name='newname[".$mid."]' value='".$newname[$mid]."' />
					...
					while ($fblock = $xoopsDB->fetchArray($fresult)) {
						...
						if (!$result) {
							...
							if ($template != '') {
								...
								if (is_array($tplfile)) {
									$tplfile[0]->setVar('tpl_source', $content, true);
									$tplfile[0]->setVar('tpl_desc', $blocks[$i]['description'], true);
									$tplfile[0]->setVar('tpl_lastmodified', time());
									$tplfile[0]->setVar('tpl_lastimported', 0);
									if (!$tplfile_handler->insert($tplfile[0])) {
