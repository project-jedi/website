...
$result = $xoopsDB->queryF("SELECT storyid FROM ".$xoopsDB->prefix(_MI_JGRAPHICS_PREFIX)." WHERE homepage=1");
...
list($storyid,$title,$text,$visible,$nohtml,$nosmiley,$nobreaks,$nocomments,$link,$address) = $xoopsDB->fetchRow($result);
      ...
      if ($link == 1) {
		...
		if (file_exists($includeContent)){
		  $xoopsOption['template_main'] = 'jgraphics_index.html';
		  ...
		  $xoopsTpl->assign('mail_link', 'mailto:?subject='.sprintf(_JGRAPHICS_INTARTICLE,$xoopsConfig['sitename']).'&amp;body='.sprintf(_JGRAPHICS_INTARTFOUND, $xoopsConfig['sitename']).':  '.XOOPS_URL.'/modules/jgraphics/index.php?id='.$id);
		  $xoopsTpl->assign('lang_printerpage', _JGRAPHICS_PRINTERFRIENDLY);
		  $xoopsTpl->assign('lang_sendstory', _JGRAPHICS_SENDSTORY);
		  $xoopsTpl->assign('id', $id);
    	}
		else{
			redirect_header("index.php",1,_JGRAPHICS_FILENOTFOUND);
		}
	  }
	  else {
        // tiny content
