...
if ($id != 0) {
  $result = $xoopsDB->queryF("SELECT storyid, title, text, visible, nohtml, nosmiley, nocomments, link, address FROM ".$xoopsDB->prefix(_MI_TINYCONTENT_PREFIX)." WHERE storyid=$id");
}
else {
//$result = $xoopsDB->queryF("SELECT storyid, title, text, homepage, nohtml, nosmiley, link, address FROM ".$xoopsDB->prefix(_MI_TINYCONTENT_PREFIX)." WHERE homepage=1");
...
list($storyid,$title,$text,$visible,$nohtml,$nosmiley,$nocomments,$link,$address) = $xoopsDB->fetchRow($result);
      ...
      if ($link == 1) {
                ...
                if (file_exists($includeContent)){
                  $xoopsOption['template_main'] = 'tinycontent_index.html';
