...
class ApiDelete extends ApiBase {
	...
	public function execute() {
		...
		if ($titleObj->getNamespace() == NS_FILE) {
			...
			$suppress = (isset($params['suppress']) ? $params['suppress'] == 'true' : false);
			$retval = self::delete($articleObj, $params['token'], $suppress, $reason);
	...
	private static function getPermissionsError(&$title, $token, $suppress = false) {
		global $wgUser;
		...
		if ($suppress) {
			$errors = $title->getUserPermissionsErrors('suppressrevision', $wgUser);
            if (count($errors) > 0) return $errors;
		}
